...
/**
 * 在多线程编程里面一个重要的概念是锁定, 如果一个资源是多个线程共享的, 
 * 为了保证数据的完整性, 在进行事务性操作时需要将共享资源锁定,
 * 这样可以保证在做事务性操作时只有一个线程能对资源进行操作, 从而保证数据的完整性. 
...
public class Lockers {
    ...
    public static class ReadWriteLockTest {
        ...
        /**
         * 获得信息. 当有线程在调用addValue方法时, getInfo得到的信息可能是不正确的. 所以, 也必须保证该方法在被调用时,
         * 没有方法在调用addValue方法.
         */
    ...
    public static void testReadWriteLockTest() throws Exception {
        ...
        // 最后一个任务是task_1
        future_1 = cachedService_1.submit(task_1);
        // 这5个任务的执行顺序可能是(可能会因为线程的调度出现写读读写写):
